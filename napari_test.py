...
grain_factor = 10
path_data = "/home/pablo/Desktop/PhD/projects/Data/gastruloids/ulla/240628_75_pos_3/preprocessed/"
path_vectors = "/home/pablo/Desktop/PhD/projects/Data/gastruloids/ulla/240628_75_pos_3/nonlinear_vectorfield_dt_{:d}/".format(grain_factor)

zres = 2.0
xyres = 0.341
start = 0
nfiles = 40
fileformat = "t{:04d}_H2BemiRFP670"

hyperstack, _ = read_split_times(path_data,  range(start+1, nfiles, grain_factor), fileformat, extension=".tif")
shape = list(hyperstack.shape)
shape[0] = 1
hyperstack = np.concatenate([np.zeros(shape,dtype=np.uint8), hyperstack],axis=0)
...
vectorformat = "vf_{:04d}_{:04d}"
mask = hyperstack[1:,:,0,:,:] > 20
vectors = read_split_vectors(path_vectors, range(start, nfiles, grain_factor), mask, vectorformat)

vector_layer = viewer.add_tracks(
    vectors,
    name="vectors",
    scale=(
        zres,
